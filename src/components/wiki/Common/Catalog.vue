...
<script>
  ...
  //CatalogNode构造函数，文章标题组成的目录节点？
  class CatalogNode {
    constructor (text, element) {
      this.text = text;//文章标题
      this.element = element;//用来返回文章的位置
    }
  }
  //把标题加入到列表中
  ...
  export default {
    ...
    methods: {
      buildCatalog () {
        this.$nextTick(() => {
          let doms = document.querySelectorAll(`#${this.domId} h1,#${this.domId} h2,#${this.domId} h3`) //返回指定选择器的所有元素
          let result = []
          addHeaderToArr(result, doms, 0, 2)//把dom中的标题都加入到一个数组中
          this.nodeArr = result//把这个数字赋值给nodeArr
        })
      },
      toTop () {
        window.scrollTo(0,0)
