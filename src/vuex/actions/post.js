...
const actions = {
  ...
  editPostTitle :({ dispatch }) => {
    dispatch(types.POST_TITLE_EDIT);
  },
  savePostTitle :({ dispatch }) => {
    dispatch(types.POST_TITLE_SAVE);

  },
  deletePost :({ dispatch,state }) => {
  ...
  submitPostTitle :({ dispatch, state },title) => {
  ...
  submitPostExcerpt :({ dispatch}, excerpt,time) => {
